...
def rating_price_inf(): #restaurant name, mich stars, trip advisor rating, infatuation rating
    q = session.query(Restaurant.name, Infatuation.price, Infatuation.rating).join(Infatuation).group_by(Restaurant.name).order_by(Restaurant.name).all()
...
def rating_price_ta(): #restaurant name, mich stars, trip advisor rating, infatuation rating
    q = session.query(Restaurant.name, TripAdvisor.rating, TripAdvisor.price).join(TripAdvisor).group_by(Restaurant.name).order_by(Restaurant.name).all()
